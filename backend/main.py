...
from agent_workflow import generate_agent_plan, PlanGenerationError
...
async def plan_from_text(text: str = Form(...)):
    ...
    if not text.strip() or len(text) > 20000:
        raise HTTPException(status_code=422, detail="Note must contain 1–20000 characters.")
    try:
        plan = await run_in_threadpool(generate_agent_plan, text)
    except PlanGenerationError:
        raise HTTPException(status_code=502, detail="Unable to generate a valid plan. Please try again.")
